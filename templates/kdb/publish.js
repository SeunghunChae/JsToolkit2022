...
function publish(symbolSet) {	
	...
	//print("publish.docsIndex"+publish.docsIndex);
	//print(typeof(publish.docsIndex));
    ...
    function addSymbol(s) {
        ...
        if (!tmp[n]) {
            tmp[n] = { name: s.name,
					   ...
					   src:s.srcFile,
					   isa:s.isa};
					   //print("if : "+s.name+ " "+s.isa);
        } else {
            tmp[n].symbols.push(s);
			//print("else :"+s.name+ " "+s.isa);
        }
    }
    ...
    function addMembers(c, lst) {
        if(lst){
            lst.filter(function(x){
                return x.memberOf == (c.alias && !x.isNamespace && x.isa=="FUNCTION") || (!x.isIgnored && x.isa=="FUNCTION");
            }).forEach(addSymbol);
        }
    }
